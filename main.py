...
CP_TOKEN = "526176:AASOM7OVc65IQGln05FUu4cn6Jx2EtXJRFx" # ВСТАВЬ ТОКЕН ИЗ @CryptoBot
...
async def project_chat_redirect(m: types.Message):
    # Обновленный текст по твоему запросу
    new_txt = (
        "💬 <b>Чат проекта DRH CASINO</b>\n\n"
        "🔗 Перейти в чат - @drhcasino_chat\n\n"
        "💎 <b>Именно в этом чате игроки играют между собой в наше казино!</b>"
    )
    kb = types.InlineKeyboardMarkup().add(
        types.InlineKeyboardButton("➡️ ПЕРЕЙТИ В ЧАТ", url=CHAT_LINK)
    )
    await m.answer(new_txt, reply_markup=kb)

@dp.message_handler(lambda m: m.text == "👛 Кошелек")
async def wallet(m: types.Message):
    u = get_user(m.from_user.id)
    bal = u['balance'] if u else 0
    txt = (f"<b>👛 Кошелек</b>\n\n🪙 <b>Ваш баланс:</b> {bal} RUB\n"
           f"🆔 <b>ID:</b> <code>{m.from_user.id}</code>\n🖥️ <b>Ник:</b> {m.from_user.full_name}")
...
async def dep_init(c: types.CallbackQuery):
    ...
    await c.message.answer(f"<b>⚡️ ПОПОЛНЕНИЕ ЧЕРЕЗ {asset}</b>\n\nВведите сумму в рублях (мин. 100):")
