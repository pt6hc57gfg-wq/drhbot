...
SUPABASE_KEY = "sb_publishable_qRT0bMlQ7XpTPC4ubSUuAg_6OryKwjs"
...
# Фикс: создаем клиент без лишних аргументов
...
async def process_dep(m: types.Message, state: FSMContext):
    ...
    kb = types.InlineKeyboardMarkup().add(
        types.InlineKeyboardButton("✅ Да", callback_data=f"adm_p_{m.from_user.id}_{amount}"),
        types.InlineKeyboardButton("❌ Нет", callback_data=f"adm_r_{m.from_user.id}")
    )
    await bot.send_message(ADMIN_ID, f"🔔 Пополнение {amount}₽ от {m.from_user.id}", reply_markup=kb)
...
async def admin_action(c: types.CallbackQuery):
    ...
    if act == "p":
        ...
        try: 
            await bot.send_photo(GROUP_ID, photo=IMG_TOPUP_CHAT, caption=f"🔥 Пополнение на {amt} RUB!")
        except: 
            pass
...
async def top(m: types.Message):
    try:
        ...
        await m.answer("Ошибка загрузки ТОПа.")
